...
genomic clusters of same-family members and split members into clustered (clusters
of >= 2 same-family members) vs singleton (mapped members not in a cluster).

iteration14: KRAB-ZNF (DBD_family_fine contains "KRAB") and HOX (HOX[ABCD]\\d) members
are REMOVED UP FRONT, before clustering, from BOTH groups (see CLAUDE.md). The clean
KRAB/HOX-excluded catalogs are the canonical inputs to the Part II/III OR scripts:
    results/clustered_paralogs.noKRABHOX.tsv
    results/singleton_paralogs.noKRABHOX.tsv
The full, un-excluded catalogs are still written for audit/provenance:
    results/clustered_paralogs.full.tsv
    results/singleton_paralogs.full.tsv
Removing KRAB/HOX before (rather than after) clustering means a rare non-KRAB/HOX
member that was only "clustered" by virtue of sitting next to a KRAB/HOX neighbour is
re-evaluated and may become a singleton -- the catalogs stay internally consistent
(every clustered row genuinely has >= 2 surviving same-family members).
...
# ---------------------------------------------------------------------------
# 3. Cluster per family per chromosome  (reusable; run on full + KRAB/HOX-excluded)
# ---------------------------------------------------------------------------
def build_clusters(members_df):
    """Return (clusters_df, singletons_df) for the given member table."""
    clusters = []          # one row per cluster (>=2 members)
    singleton_rows = []    # mapped members not in a multi-member cluster
    cluster_id = 0
    for family, fam in members_df.groupby("family"):
...
def write_outputs(cl, sg, clust_path, sing_path, tag):
    cl.to_csv(clust_path, sep="\t", index=False)
    sg.to_csv(sing_path, sep="\t", index=False)
    ...
    log(f"  wrote {clust_path.name} + {sing_path.name}")

# ---------------------------------------------------------------------------
# 4. Full catalog (audit) then KRAB/HOX-excluded catalog (canonical)
# ---------------------------------------------------------------------------
cl_full, sg_full = build_clusters(M)
write_outputs(cl_full, sg_full,
              RES / "clustered_paralogs.full.tsv",
              RES / "singleton_paralogs.full.tsv", "FULL (KRAB/HOX retained)")

# iteration14: remove KRAB-ZNF (fine family contains "KRAB") + HOX (HOX[ABCD]\d) up front
is_krab = M["family"].astype(str).str.contains("KRAB", case=False, na=False)
is_hox  = M["gene_symbol"].astype(str).str.match(r"HOX[ABCD]\d", na=False)
drop = is_krab | is_hox
log("")
log(f"removing KRAB-ZNF ({int(is_krab.sum())}) + HOX ({int(is_hox.sum())}) "
    f"= {int(drop.sum())} mapped members before clustering")
M_clean = M[~drop].reset_index(drop=True)
log(f"  members remaining after KRAB/HOX removal: {len(M_clean)} (of {len(M)})")

cl, sg = build_clusters(M_clean)
write_outputs(cl, sg,
              RES / "clustered_paralogs.noKRABHOX.tsv",
              RES / "singleton_paralogs.noKRABHOX.tsv", "KRAB/HOX-EXCLUDED (canonical)")

log("")
log("families contributing the most clusters (KRAB/HOX-excluded):")
if len(cl):
