#!/usr/bin/env python3
"""
CYTOKINE-RESPONSE DEG enrichment OR across the FOUR duplication categories (script 06),
the cytokine-dictionary analog of script 07 (which used steady-state lineage markers).

   dispersed_ohnolog | clustered_cis_ohnolog | clustered_SSD_tandem | dispersed_SSD

Marker = paralogous TF that is a cytokine RESPONSE DEG (|log2FC|>=1 & FDR<0.05, gated in
script 09) to >=1 cytokine in >=1 cell type (results/intermediate/human_cytokine_dict_DEGs.tsv,
`sheet` = cell type). Universe = cleaned 4-group catalog (Lambert TFs, KRAB/HOX/readthrough/
pseudo removed) intersected with PBMC-detectable genes (read directly from human_pbmc.h5ad
var_names, backed -- same universe rule as scripts 07/08).

Same statistics as script 07: per-category one-vs-rest Fisher OR (+CI, p, BH-FDR), the key
provenance/arrangement pairwise contrasts, and a provenance x arrangement factorial logit.

Outputs:
  results/TF_dup4_cytokine_DEG_OR.summary.tsv
  results/TF_dup4_cytokine_DEG_OR.by_celltype.tsv
  results/TF_dup4_cytokine_DEG_OR.per_TF.tsv
  results/figures/TF_dup4_cytokine_DEG_OR.{pdf,png}
Run: /mnt/alvand/apps/anaconda2/envs/py3/bin/python3 scripts/10_human_cytokine_dup4_DEG_OR.py
"""
...
OUT_SUM = RES / "TF_dup4_cytokine_DEG_OR.summary.tsv"
OUT_CT = RES / "TF_dup4_cytokine_DEG_OR.by_celltype.tsv"
OUT_PER = RES / "TF_dup4_cytokine_DEG_OR.per_TF.tsv"
OUT_FIG = RES / "figures" / "TF_dup4_cytokine_DEG_OR"
...
def main():
    ...
    cell_types = sorted(d["sheet"].unique())
    resp_by_ct = {ct: set(d.loc[d["sheet"] == ct, "human_symbol"]) for ct in cell_types}
    any_resp = set().union(*resp_by_ct.values())

    genes = u.gene.values
    u["is_responder"] = np.isin(genes, list(any_resp))
    u["n_response_celltypes"] = [sum(g in resp_by_ct[ct] for ct in cell_types) for g in genes]
    u.to_csv(OUT_PER, sep="\t", index=False)

    marker = u.is_responder.values
    ...
    for _, r in ovr.iterrows():
        summ.append(dict(test=r.test, OR=r.OR, ci_lo=r.ci_lo, ci_hi=r.ci_hi, p=r.p,
                         fdr=r.fdr, note=f"n={r.n}, responder_rate={r.marker_rate:.3f} ({r.note})"))
    ...
    # (4) per-cell-type responder rate by category
    ctrows = []
    for ct in cell_types:
        mk = np.isin(genes, list(resp_by_ct[ct]))
    ...
    log("per-category one-vs-rest (cytokine response):")
...
def make_figure(u, ovr, sdf):
    ...
    for i, (rt, n, orr, q) in enumerate(zip(rates, ns, ors, fdr)):
        ax.text(i, rt + 0.005, f"{rt:.2f}{'*' if q<0.05 else ''}\nn={n}\nOR={orr:.2f}", ha="center", fontsize=8)
    ax.set_xticks(range(4))
    ax.set_xticklabels(["dispersed\nohnolog", "clustered\ncis-ohnolog",
                        "clustered\nSSD-tandem", "dispersed\nSSD"], fontsize=8.5)
    ax.set_ylabel("fraction = cytokine-response DEG (any cell type)")
    ax.set_title("(A) Cytokine-responder rate by 4 categories\n(one-vs-rest OR; * FDR<0.05)", fontsize=10)
    ...
    for i, pv in enumerate(["ohnolog", "SSD"]):
        for j, ar in enumerate(["clustered", "dispersed"]):
            ...
            grid[i, j] = u.loc[m, "is_responder"].mean() if m.sum() else np.nan
            ngrid[i, j] = int(m.sum())
    im = ax.imshow(grid, cmap="Purples", vmin=0, vmax=np.nanmax(grid) * 1.05)
    for i in range(2):
        for j in range(2):
    ...
    ax.set_title("(B) Responder rate: provenance x arrangement", fontsize=10)
    fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04).set_label("responder rate", fontsize=8)
    ...
    fig.suptitle("PBMC CYTOKINE-RESPONSE DEG enrichment across the 4 duplication categories "
                 "(Oesinghaus dictionary; Lambert TFs; KRAB/HOX/readthrough/pseudo removed)",
